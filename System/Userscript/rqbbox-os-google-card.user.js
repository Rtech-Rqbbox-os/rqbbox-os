// ==UserScript==
// @name         RQBBOX OS — Google Search Info Card
// @namespace    https://github.com/Rtech-Rqbbox-os/rqbbox-os
// @version      1.2.0
// @description  Shows the RQBBOX OS info card directly on Google Search results when you search for RQBBOX OS. Free, no installation needed — just install Tampermonkey then click this script.
// @author       RhysTech (rhyscotton20@gmail.com)
// @match        https://www.google.com/search*
// @match        https://google.com/search*
// @match        https://www.googlechrome.com/search*
// @match        https://googlechrome.com/search*
...
(function() {
  ...
  function buildCard() {
    ...
    return '<div id="' + ID + '" style="background:linear-gradient(135deg,rgba(20,22,28,.98),rgba(10,12,18,.98));backdrop-filter:blur(20px) saturate(1.4);-webkit-backdrop-filter:blur(20px) saturate(1.4);border:1px solid rgba(0,212,255,.12);border-radius:12px;padding:0;margin:12px 0;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,Helvetica,Arial,sans-serif;color:#fff;max-width:652px;box-shadow:0 8px 32px rgba(0,0,0,.6);overflow:hidden">' +
      ...
      '</div>' +
      '<div style="padding:12px 18px;font-size:13px;line-height:1.6;color:rgba(255,255,255,.55);border-bottom:1px solid rgba(255,255,255,.04)">' + DATA.description + '</div>' +
      ...
      '</div>' +
      '<div style="margin:0 18px 10px;border-top:1px solid rgba(255,255,255,.04)">' + featureRows + '</div>' +
      '<div style="padding:10px 18px 14px;display:flex;flex-wrap:wrap;gap:6px;border-top:1px solid rgba(255,255,255,.04)">' +
        '<a href="' + DATA.download + '" target="_blank" style="display:inline-flex;align-items:center;gap:6px;padding:8px 18px;border-radius:8px;font-size:13px;font-weight:600;text-decoration:none;background:linear-gradient(135deg,#007bff,#00d4ff);color:#fff;box-shadow:0 4px 16px rgba(0,212,255,.25)">⬇ Download ' + DATA.name + '</a>' +
      ...
      '<div style="padding:8px 18px;background:rgba(0,0,0,.2);border-top:1px solid rgba(255,255,255,.04);display:flex;justify-content:space-between;flex-wrap:wrap;gap:4px;align-items:center">' +
        '<div style="font-size:10px;color:rgba(255,255,255,.2)"><span>' + DATA.license + ' &bull; ' + DATA.platforms + '</span></div>' +
  ...
  function inject() {
    ...
    var targets = ['#rhs','#rhs_block','#search','#center_col','#rso','.srg','div[data-async-context]','#main'];
    var container = null;
  ...
  inject();
  window.addEventListener('load', function() { setTimeout(inject, 600); });
})();
